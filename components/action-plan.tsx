...
export function ActionPlan({ steps }: { steps: ActionStep[] }) {
  return (
    <SectionCard id="action-plan" title="Action Plan" description="A specific sequence you can start this week">
      <ol className="relative ml-4 border-l border-violet-200">
        {steps.map((step, index) => (
          <li key={`${step.title}-${index}`} className="relative pb-8 pl-8 last:pb-0">
            ...
            <p className="mt-3 rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-600"><strong className="text-slate-800">Outcome:</strong> {step.outcome}</p>
          </li>
        ))}
      </ol>
    </SectionCard>
